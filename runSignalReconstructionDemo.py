...
# length of the signal to be recovered, and the options.  Note, the measurement
# operator can be either a function handle or a matrix.   Here, we use a
# matrix.  In this case, we have omitted the second argument. If 'A' had been a
# function handle, we would havei handed the transpose of 'A' in as the second
# argument.
x, outs, opts = solvePhaseRetrieval(A=A, b0=b, opts=opts)
